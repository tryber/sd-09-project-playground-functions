...
function highestCount(numbersArray) {
  // seu código aqui
  let highest = 0;
  let highestEqualsCount = 0;
  for (let pos = 0; pos < numbersArray.length; pos += 1) {
    if (numbersArray[pos] >= highest) {
      highest = numbersArray[pos];
    }
  }
  for (let pos = 0; pos < numbersArray.length; pos += 1) {
    if (highest === numbersArray[pos]) {
      highestEqualsCount += 1;
    }
  }
